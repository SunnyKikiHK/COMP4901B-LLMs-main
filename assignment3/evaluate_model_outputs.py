...
def estimate_pass_at_k(
    ...
    if isinstance(num_samples, int):
        num_samples = np.array([num_samples] * len(num_correct))
    else:
        num_samples = np.array(num_samples)
    num_correct = np.array(num_correct)

    pass_at_k_values = np.zeros_like(num_correct, dtype=float) 
    ...
    for i in range(len(num_correct)):
        ...
        not_c = n - c
        if not_c < k:
            pass_at_k_values[i] = 1 # If not enough wrong samples, pass@k is 1
            continue
        
        indices = np.arange(k) #shape: (k,)
        prob_all_wrong = np.prod((not_c - indices) / (n - indices))

        pass_at_k_values[i] = 1 - prob_all_wrong
...
def extract_solution(solution_str: str) -> Optional[str]:
    ...
    numbers = re.findall(r'\\boxed\{([^}]+)\}', solution_str)
    if not numbers:
        # Strategy 2: extract the last number in the string
        numbers = re.findall(r'(\-?[0-9\.\,]+)', solution_str) # \-?: optional minus sign
        if not numbers:
            # Strategy 4: No numbers found
            return None
    # Strategy 3: take the last number found 
    answer_candidate = numbers[-1]

    extracted_solution = answer_candidate.replace(',', '').strip().rstrip('.')
    # =======================================================================
...
def compute_score(solution_str: str, ground_truth: str) -> int:
    ...
    # =======================================================================
    try:
        if str(float(answer)) == str(float(ground_truth)):
            is_correct = 1
        else:
            is_correct = 0  
        # =======================================================================
